 ...
 */
function checkIfThereIsUserWithGivenEmail($email)
{
    return count(array_filter($_SESSION['users'], function ($user) use ($email) {
        return $user['email'] === $email;
    })) > 0;
}

if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (checkIfThereIsUserWithGivenEmail($_POST['email'])) {
