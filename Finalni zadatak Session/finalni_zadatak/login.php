 ...
 */
function checkIfCredentialsAreValid(string $email, string $password): bool
{
    return count(array_filter($_SESSION['users'], function ($user) use ($email, $password) {
        return $user['email'] === $email && $user['password'] === $password;
    })) > 0;
}

if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (checkIfCredentialsAreValid($_POST['email'], $_POST['password'])) {
